...
public class ObjdumpSystemLibraryARM32 {
	...
	private void writeObjdump() throws IOException {
		File script = new File(OutputDirectory + "objdump_arm.sh");
		if (script.exists())
			script.delete();
		...
		fw.write("if [ ! -d \"" + Configuration.Workspace + Configuration.ArmSystemLibDirectory + "\" ]; then" + "\n");
		fw.write("\t" + "mkdir " + Configuration.Workspace + Configuration.ArmSystemLibDirectory + "\n");
		fw.write("fi" + "\n");
		fw.write("\n");
		...
		for (File library : libraries) {
			...
			String objdumpPath = Configuration.Workspace + Configuration.ArmSystemLibDirectory + librarySubPath.replace("/", "_").replace(".so", ".txt");
	...
	public static void main(String[] args) {
		ObjdumpSystemLibraryARM32 generator = new ObjdumpSystemLibraryARM32();
